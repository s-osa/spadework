...
describe "Filter: set_status_filter" do
  describe "when Order::Amazon" do
    before { @order = Order::Amazon.new([]) }

    it "should set nothing as status regardless of payment method." do
      @order.stub(:payment_method).and_return(:card)
      @order.set_status_filter
      @order.status.empty?.should == true
    end
  end

  describe "else (like Order::Rakuten, Order::Yahoo)" do
